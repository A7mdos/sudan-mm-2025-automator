...
def refresh_token():
    ...
    try:
        ...
        if not creds.refresh_token:
            ...
            print("\nYour token was created without a refresh token.")
            print("You need to re-authenticate to get one:")
            print("  1. Delete token.pickle")
            print("  2. Run: streamlit run app.py")
            print("  3. Complete OAuth authentication (a browser window will open)")
        ...
        if creds.valid and not creds.expired:
            ...
            print("Running get_token_for_secrets.py to extract current token data...\n")
        else:
            print("\nRefreshing token...")
        ...
        # Display token data (works for both valid and just-refreshed tokens)
        token_data = {
            'token': creds.token,
        ...
        print("\n" + "=" * 60)
        print("TOKEN DATA FOR STREAMLIT CLOUD")
        print("=" * 60)
